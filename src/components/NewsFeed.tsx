    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex border-b mb-8">
        {TABS.map(tab => (
          <button
            key={tab.key}
            className={`px-4 py-2 -mb-px font-semibold border-b-2 transition-colors ${activeTab === tab.key ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-blue-600'}`}
            onClick={() => setActiveTab(tab.key as 'recommended' | 'all')}
          >
            {tab.label}
          </button>
        ))}
